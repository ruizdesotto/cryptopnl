...
def test_fifo_with_trades_process_all_trades(fifo_with_trades_fixture, mocker):
    """
    Asserts all trades are processed  
    """

    mock_process = mocker.patch("cryptopnl.main.fifo_with_trades.fifo_with_trades.process_trade", return_value=True)

    fifo_with_trades_fixture.process_all_trades()
    t0 = fifo_with_trades_fixture._trades._trades.iloc[0]
    t1 = fifo_with_trades_fixture._trades._trades.iloc[1]
    t2 = fifo_with_trades_fixture._trades._trades.iloc[2]
    t3 = fifo_with_trades_fixture._trades._trades.iloc[3]
...
def test_fifo_with_trades_pnl_summary(fifo_with_trades_fixture, capsys):
    ...
    fifo_with_trades_fixture.fifo_gains = all_gains

    pnl = fifo_with_trades_fixture.pnl_summary()

    capture = capsys.readouterr()
    assert pnl == {"2020": 3.0, "2021": 30.0}
    assert re.match('2020(.*)3.0(\n)*(.*)2021(.*)30.0', capture.out)

def test_fifo_with_trades_go(fifo_with_trades_fixture, mocker):
    """
    Assert the functions are called
    """

    mock_process = mocker.patch("cryptopnl.main.fifo_with_trades.fifo_with_trades.process_all_trades", return_value=True)
    mock_summary = mocker.patch("cryptopnl.main.fifo_with_trades.fifo_with_trades.pnl_summary", return_value=True)

    fifo_with_trades_fixture.go()
